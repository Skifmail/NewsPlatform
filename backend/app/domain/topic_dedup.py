...
# Частотный «филлер»: длинные слова, встречающиеся во множестве РАЗНЫХ тем.
# Их совпадение НЕ означает, что тема повторяется, поэтому они не считаются
# отличительным субъектом (иначе любые две темы про «секрет/систему/науку»
# ложно схлопывались бы в дубликаты).
_GENERIC_WORDS: Final[frozenset[str]] = frozenset(
    {
        # общенаучный / общий филлер
        "наука", "науки", "науку", "закон", "законы", "законам", "законов",
        "физика", "физики", "физику", "химия", "химии", "секрет", "секреты",
        "секрета", "тайна", "тайны", "тайну", "феномен", "феномена", "эффект",
        "эффекты", "система", "системы", "систему", "природа", "природы",
        "человек", "человека", "люди", "людей", "земля", "земле", "землю",
        "вода", "воды", "воде", "свет", "света", "время", "времени", "мир",
        "мира", "правда", "правды", "факт", "факты", "истина", "история",
        "истории", "историю", "культура", "культуры", "загадка", "загадки",
        "секретный", "настоящий", "простой", "главный", "самый", "древний",
        "древние", "древних", "почему", "разоблачаем", "миф", "мифы",
        # частые глаголы — не субъект темы (иначе «спутники не падают» ↔
        # «небоскрёбы не падают» ложно схлопываются)
        "падают", "падает", "падать", "летает", "летают", "летать",
        "обманул", "обманула", "стоят", "стоит", "считают", "считает",
        "слышит", "слышат", "дышат", "дышит", "поит", "потерял",
        "потеряла", "спасший", "спасли", "живут", "живет", "умеет",
        "умеют", "знают", "знали", "используют", "остается", "остаётся",
        "плавает", "плавают", "течет", "течёт", "делают", "делает",
        # страны/гео — баланс делает промпт, а не жёсткий дедуп
        "япония", "японии", "японию", "японцы", "японские", "японский",
        "россия", "россии", "италия", "италии", "швейцария", "швейцарии",
        "марокко", "эфиопия", "эфиопии", "амазония", "амазонии",
        "антарктида", "антарктиды", "греки", "греция", "греции",
        # технический филлер (канал находок)
        "агент", "агента", "агенты", "агентов", "терминал", "инструмент",
        "инструменты", "проект", "проекты", "данные", "данных", "модель",
        "модели", "нейросеть", "приложение", "сервис", "утилита", "утилиты",
        "библиотека", "github", "opensource", "source", "open",
    }
)
...
def is_topic_too_similar(candidate: str, recent: list[str]) -> bool:
    ...
    for prev in recent:
        ...
        # Пересечение отличительных слов-субъектов. Требуем НЕ МЕНЕЕ ДВУХ общих
        # слов: одного общего слова недостаточно (на списке из 25–50 недавних
        # тем почти любая новая тема делит хотя бы одно слово — особенно у
        # канала находок, где описания репозиториев полны общих тех-слов, — и
        # тогда идеация ложно отвергает все варианты и падает). Точечные
        # субъект-дубли ловит `_shared_rare_words`, близость — overlap_ratio.
        if len(_distinctive_words(candidate) & _distinctive_words(prev)) >= 2:
            return True
...
def _distinctive_words(text: str) -> set[str]:
    """Отличительные слова-субъекты: значимые, но не частотный филлер.

    Args:
        text: тема или заголовок.

    Returns:
        set[str]: слова длиной от 4 символов, не входящие в общий филлер.
    """
    return {w for w in _significant_words(text) if w not in _GENERIC_WORDS}


def _word_overlap_ratio(a: str, b: str) -> float:
    """Доля пересечения отличительных слов двух тем.

    Считает только слова-субъекты (без частотного филлера), чтобы общие
    «секрет/тайна/наука» не завышали похожесть разных тем.
