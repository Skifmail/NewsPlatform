...
def is_topic_too_similar(candidate: str, recent: list[str]) -> bool:
    ...
    for prev in recent:
        ...
        # Общее отличительное слово-субъект (напр. «шмель», «пирамиды») —
        # сильный признак той же темы, даже если остальные слова разные.
        # Считаем только отличительные слова, а не частотный филлер
        # (секрет/тайна/наука), иначе разные темы ложно схлопываются.
        if _distinctive_words(candidate) & _distinctive_words(prev):
            return True
