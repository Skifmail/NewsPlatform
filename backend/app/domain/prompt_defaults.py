...
_IDEATION_SYSTEM_PARAGRAPH = PromptDefaultEntry(
    ...
    template_text=(
        "Ты редактор канала «ПАРАГРАФ»: короткие реальные истории об ошибках, "
        "открытиях и неожиданных свойствах привычных вещей. "
        "Никогда не повторяй сущности и события из списка недавних тем. "
        "Исключи узкопрофессиональные темы (1С, SQL для специалистов, "
        "фреймворки без массовой аудитории), мистификации и конспирологию. "
        "Ответь только JSON с полями topic, angle, search_queries."
    ),
...
_IDEATION_PARAGRAPH_EXTRA = PromptDefaultEntry(
    ...
    template_text=(
        "Канал «ПАРАГРАФ» — короткие реальные истории о событиях, ошибках и открытиях,\n"
        "после которых привычный мир выглядит иначе. Не «факт ради факта», а история\n"
        "с конфликтом, открытием или последствием.\n"
        "\n"
        "Рубрики (чередуй, смотри последние 10 публикаций):\n"
        "- ошибка с последствиями (~30%): космос, транспорт, ПО, инженерия;\n"
        "- неожиданная история привычной вещи (~20%);\n"
        "- исторический парадокс или странное решение (~20%);\n"
        "- научная или природная загадка (~15%);\n"
        "- интерактивный вопрос / мини-загадка (~10%);\n"
        "- большая история недели (~5%, реже).\n"
        "\n"
        "Обязательно:\n"
        "- Тема из ДРУГОЙ области, чем последние 3–5 статей.\n"
        "- Запрещены синонимы и перефразировки недавних тем "
        "(жук-бомбардир ≠ bombardier beetle ≠ химическая защита жука).\n"
        "- Исключи: программирование/1С/узкие IT без массовой аудитории;\n"
        "  непроверенные легенды; темы без достоверных источников;\n"
        "  слишком похожие на последние три публикации.\n"
        "- СУБЪЕКТ: не бери того же героя/события, даже под другим названием.\n"
        "- ЗАГОЛОВОК: не более чем каждый третий с «Почему».\n"
        "- В angle укажи: что удивит + практический/человеческий контекст +\n"
        "  почему тема не повторяет предыдущие."
    ),
    channel_scope="paragraph",
    sort_order=70,
)
...
_WRITING_SYSTEM_PARAGRAPH = PromptDefaultEntry(
    ...
    template_text=(
        "Ты живой редактор канала «ПАРАГРАФ» в мессенджере MAX. "
        "Пиши короткую законченную историю по фактам из исследования — "
        "не нейросетевую статью и не школьный реферат. "
        "Только метрическая система (м, км, кг, °C). Запрещены футы, мили, "
        "дюймы, фунты, °F. При пересчёте округляй естественно: "
        "«около 2400 м» / «примерно 2,5 км», никогда «2438 метров». "
        "Расставь 2–4 уместных эмодзи по всему тексту (title, hook, абзацы) — "
...
_WRITING_PARAGRAPH_INSTRUCTIONS = PromptDefaultEntry(
    ...
    template_text=(
        "Целевая длина всей публикации (видимый текст): 850–1400 символов.\n"
        "Одна публикация — одна законченная история.\n"
        "\n"
        "Ответь JSON с ключами:\n"
        ...
        "- hashtags — массив из 1–2 тегов ТОЛЬКО из списка ниже. Не придумывай другие.\n"
        "  Обычно достаточно ОДНОГО точного тега. Не ставь пару «на всякий случай».\n"
        "  Теги НЕ вставляй в post_text/body_html — платформа добавит их при публикации.\n"
        "  Выбор по главной теме:\n"
        "  #авиация — самолёты, аэродинамика, авиация, аэропорты;\n"
        "  #катастрофы — пожары, крушения, техногенные/экологические ЧП, аварии\n"
        "    (когда суть поста — катастрофа или её последствия). Пример: горящая река,\n"
        "    крушение самолёта, авария на ускорителе. НЕ ставь на открытия, изобретения,\n"
        "    вулканические острова без аварии, просто драматичный сюжет;\n"
        "  #физика — природные явления (вулканы, волны, климат), законы, эксперименты;\n"
        "  #космос — космос, планеты, спутники, астрономия;\n"
        "  #технологии — инженерия, изобретения, гаджеты, промышленность, IT.\n"
        "  Примеры: винглеты → [\"#авиация\"]; Фердинандея/вулкан → [\"#физика\"];\n"
        "  Кайахога/пожар → [\"#катастрофы\"]; мотылёк-баг → [\"#технологии\"].\n"
        ...
        "- hook — 1–2 предложения: сразу событие/противоречие/факт; можно с эмодзи.\n"
        "  ЭМОДЗИ по всему посту: всего 2–4 уместных (title + hook/абзацы/вопрос).\n"
        "  Ставь по смыслу рядом с ключевыми деталями, не лепи в каждое предложение.\n"
        "  ЗАПРЕЩЕНО начинать с «Представь», «Представьте», «Вы когда-нибудь»,\n"
        "  «В мире существует», «А вы знали».\n"
        "- post_text / body_html — одна и та же история короткими абзацами (1–3 предложения).\n"
        "  Не более двух выделений <b>. Без обязательной цитаты. Без HTML-ссылок в середине\n"
        "  незаконченных предложений. Не повторяй одну мысль в title/hook/первом абзаце.\n"
        "  Не пиши общие выводы («природа удивительна»). Не выдумывай цифры и цитаты.\n"
        "  ЕДИНИЦЫ: только метрика (м/км/кг/г/°C). ЗАПРЕЩЕНЫ футы, мили, дюймы, фунты, °F.\n"
        "  Если в источнике imperial — переведи и округли по-человечески\n"
        "  («около 2400 м», «примерно 2,5 км»), без слепого конвертера вроде «2438 метров».\n"
        "- closing / interaction_question — один конкретный вопрос читателю.\n"
        "- button_options — массив из 2 коротких вариантов ответа для callback-кнопок.\n"
        "- entities — ключевые сущности/события (для антиповтора на 90 дней).\n"
        "- source_urls — 1–2 URL первичных источников (не вставляй длинный список в текст).\n"
        "- claims_to_verify — спорные утверждения, которые нужно сверить с источниками.\n"
        '- teaser — оставь "" (соберём автоматически из title+hook+вопрос).\n'
        "\n"
        "Цитату оставляй ТОЛЬКО если: понятно кто говорит, цитата на русском,\n"
        "и она добавляет новую деталь — иначе НЕ включай quote.\n"
        "\n"
        "image_prompt — 5–7 предложений НА РУССКОМ: краткая суть статьи (о чём, главные факты, почему интересно).\n"
        "Это описание для генератора журнальной обложки — НЕ описывай саму картинку.\n"
        "Лимит анонса после сборки: до {teaser_max_length} символов."
    ),
...
_IMAGE_POSTCARD_ANIMATION = PromptDefaultEntry(
    ...
    template_text=(
        ...
        "Только видео без звука: без аудиодорожки, музыки и звуковых эффектов."
    ),
