...
_IMAGE_WRITER_HINT_POSTCARD = PromptDefaultEntry(
    ...
    template_text=(
        "Поле image_prompt: 1 короткая фраза НА РУССКОМ про настроение или визуальный акцент открытки. "
        "Не описывай полный кадр. Избегай общих штампов вроде flowers, window, cup, dove, gold lettering, "
        "если они не нужны по смыслу темы."
    ),
    channel_scope="postcard",
    sort_order=20,
)
...
_IMAGE_COVER_PROMPT_POSTCARD = PromptDefaultEntry(
    ...
    template_text=(
        "Сделай красивую квадратную открытку 1:1 по теме «{title}».\n"
        "Смысловой акцент: {scene}\n"
        "Не повторяй шаблонные сцены и однотипные композиции. Меняй палитру, сезон, предметы, ракурс и свет под смысл повода.\n"
        "Избегай клише вроде одинаковых голубей, чашек, цветов у окна и перегруженной золотой типографики, если тема этого не требует.\n"
        "Если короткая надпись звучит естественно, используй только её: «{greeting_text}». Если нет — не добавляй текст на изображение."
    ),
