...
def is_long_form_article_channel(channel: Channel) -> bool:
    ...
    Подходит для любой платформы (Telegram, MAX): Github | Находки и ПАРАГРАФ
    в режиме ``content_mode=article``. Для MAX это критично — Telegraph
    недоступен без VPN, а MAX-аудитория читает без него.
    ...
    if channel.content_mode != ContentMode.ARTICLE.value:
        return False
    return is_devtools_article_channel(
